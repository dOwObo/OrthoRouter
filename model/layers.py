...
class LoRALayer(nn.Module):
    def __init__(self, original_layer, rank: int = 4):
        ...
        # 固定 LoRA 參數，不進行動態擴展
        self.lora_A = nn.Parameter(torch.zeros((rank, original_layer.wi.weight.size(1))))
        self.lora_B = nn.Parameter(torch.zeros((original_layer.wi.weight.size(0), rank)))
    ...
    def forward(self, hidden_states):
        ...
        lora_output = hidden_states @ self.lora_A.T @ self.lora_B.T
